//langkaloka-v1\components\views\Header.tsx
"use client";
...
function NavDrawerRow({
  ...
  const showBadge = showBadgeAlways || (badge !== undefined && badge > 0);
  return (
    <button
      onClick={onClick}
    ...
    >
      <div
        className={`flex h-8 w-8 items-center justify-center rounded-xl ${active ? "bg-blue-100" : "bg-gray-100"}`}
      >
...
export function Header() {
  ...
  const incrementNotif = useCallback(() => {
    notifRef.current += 1;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (y <= 10) setSearchBarVisible(true);
        else if (y > lastScrollY.current + THRESHOLD)
          setSearchBarVisible(false);
        else if (y < lastScrollY.current - THRESHOLD) setSearchBarVisible(true);
  ...
  useEffect(() => {
    ...
    const onUpdate = (data: ChatListUpdateEvent) => {
      ...
      if (buyerId !== me && sellerId !== me) return;
  ...
  useEffect(() => {
    if (pathname.startsWith("/chat")) resetNotif();
  }, [pathname, resetNotif]);
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/95 backdrop-blur-sm shadow-sm">
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4 lg:px-6">
          ...
          <div className="ml-auto flex items-center gap-1.5 lg:ml-0">
            {isLoading ? (
              ...
              <div className="hidden lg:flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-gray-200 bg-white pl-1 pr-3 py-1 transition hover:border-blue-200 hover:bg-blue-50 cursor-pointer"
                >
            ...
            {/* Hamburger */}
            <button
              className="relative flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-gray-100 lg:hidden cursor-pointer"
        ...
        {/* Search mobile */}
        <div
          className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${searchBarVisible ? "max-h-16 opacity-100" : "max-h-0 opacity-0"}`}
        >
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 z-50 h-dvh w-80 bg-white shadow-2xl transition-transform duration-300 ease-out lg:hidden ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
