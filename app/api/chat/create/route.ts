...
import { chats, products, stores } from "@/db/schema"
...
const existingChat = await db
  ...
  .where(
    and(
      ...
      or(
        and(
          eq(chats.buyerId, decoded.id),
          eq(chats.sellerId, sellerId)
        ),
        and(
          eq(chats.buyerId, sellerId),
          eq(chats.sellerId, decoded.id)
        )
      )
    )
  )
...
const [chat] = await db
  ...
  .values({
    productId,
    buyerId: decoded.id, // buyer selalu user login
    sellerId: sellerId   // seller dari store
  })
  .returning()
